...
void Assembly::Construct(Observer** sensor, Observer** observer){
	...
	while (getline(infile, line)&&line.compare("")!=0){
		...
		if(tokens.at(0).compare("load")==0 || tokens.at(0).compare("load_ft")==0){
			...
			observer[subformula_tag]=new Observer_type_0(sensor[obNum]);
		}else if(tokens.at(0).compare("not")==0){
			string src=tokens.at(1);
			int obNum=stoi(src.substr(1,src.length()),nullptr);
			if(src[0]=='a'){//atomic
				observer[subformula_tag]=new Observer_type_1(sensor[obNum]);
			}else{//subformula, =='s'
				observer[subformula_tag]=new Observer_type_1(observer[obNum]);
			}
			...
			if(src[0]=='a'){//atomic
				observer[subformula_tag]=new Observer_type_2(sensor[obNum],intNum);
			}else{//subformula, =='s'
				observer[subformula_tag]=new Observer_type_2(observer[obNum],intNum);
			}
			...
			if(src_1[0]=='a'&& src_2[0]=='a') observer[subformula_tag]=new Observer_type_3(sensor[obNum_1],sensor[obNum_2]);
			else if(src_1[0]=='a'&& src_2[0]=='s') observer[subformula_tag]=new Observer_type_3(sensor[obNum_1],observer[obNum_2]);
			else if(src_1[0]=='s'&& src_2[0]=='a') observer[subformula_tag]=new Observer_type_3(observer[obNum_1],sensor[obNum_2]);
			else observer[subformula_tag]=new Observer_type_3(observer[obNum_1],observer[obNum_2]);
			...
			if(src[0]=='a'){//atomic
				observer[subformula_tag]=new Observer_type_4(sensor[obNum],intNum_1,intNum_2);
			}else{//subformula, =='s'
				observer[subformula_tag]=new Observer_type_4(observer[obNum],intNum_1,intNum_2);
			}
